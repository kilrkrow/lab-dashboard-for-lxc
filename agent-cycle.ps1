...
Usage:
  .\agent-cycle.ps1              # full cycle
  .\agent-cycle.ps1 -WhatIf      # check + dry-run deploy only
  .\agent-cycle.ps1 -SkipBroker  # static only
  .\agent-cycle.ps1 -StrictApi   # fail cycle if /api/* soft-fails
  .\agent-cycle.ps1 -NoRestore   # never auto-restore (still reports)

Human one-time: copy deploy.env.example → deploy.env, fill LXC_* + LXC_SSH_KEY + HAVEN_URL.
Then tell agent: "iterate live" / "agent-cycle".
#>
...
# 1) Gate
Write-Host "`n[1/4] npm run check" -ForegroundColor Cyan
npm run check
...
# 2) Deploy static
Write-Host "`n[2/4] deploy static" -ForegroundColor Cyan
if ($WhatIf) {
...
# 3) Optional broker ship
if (-not $SkipBroker -and $env:BROKER_REMOTE_DIR) {
    Write-Host "`n[3/4] ship broker → $($env:BROKER_REMOTE_DIR)" -ForegroundColor Cyan
    . "$root\RemoteSync.ps1"
    ...
    # Ensure remote dir + copy broker source (single file is enough for runtime via tsx/node)
    $mk = Invoke-NativeExit -FilePath "ssh" -ArgumentList ($base + @(
        "${user}@${host}",
        "mkdir -p '$remoteDir/server' '$remoteDir/dist-server' 2>/dev/null; true"
    ))
    if ($mk -ne 0) { Write-Host "WARN: could not mkdir broker dir (exit $mk)" -ForegroundColor Yellow }

    $scp1 = Invoke-NativeExit -FilePath "scp" -ArgumentList ($base + @(
        (Join-Path $root "server\broker.ts"),
        "${user}@${host}:${remoteDir}/server/broker.ts"
    ))
    if ($scp1 -ne 0) {
    ...
    # Also refresh dist next to broker if it serves static itself
    if (Test-Path (Join-Path $root "dist\index.html")) {
        Write-Host "Sync dist/ into broker remote (for broker-served static)…" -ForegroundColor DarkGray
        # reuse managed sync into remoteDir/dist
        $localDist = Join-Path $root "dist"
        Invoke-SyncToRemote -LocalDir $localDist -RemoteUser $user -RemoteHost $host -RemotePath "$remoteDir/dist" -Delete
    }
    ...
    Write-Host "`n[3/4] skip broker ship (set BROKER_REMOTE_DIR in deploy.env to enable)" -ForegroundColor DarkGray
}

# 4) Smoke
Write-Host "`n[4/4] smoke" -ForegroundColor Cyan
$smokeArgs = @()
...
Write-Host "`nCYCLE OK" -ForegroundColor Green
exit 0
